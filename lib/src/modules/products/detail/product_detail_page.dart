...
class _ProductDetailPageState extends State<ProductDetailPage>
  ...
  final productDetailController = Modular.get<ProductDetailController>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      statusDisposer =
          reaction((_) => productDetailController.status, (status) {
        switch (status) {
          ...
          case ProductDetailStateStatus.loaded:
            final model = productDetailController.productModel!;
          ...
          case ProductDetailStateStatus.error:
            hideLoader();
            showError(productDetailController.errorMessage!);
            break;
      ...
      productDetailController.loadProduct(widget.productId);
    });
  }
                ...
                children: [
                  Stack(
                    ...
                    children: [
                      Observer(
                        builder: (_) {
                          if (productDetailController.imagePath != null) {
                            return Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Image.network(
                                '${Env.instance.get('backend_base_url')}${productDetailController.imagePath}',
                                width: 200,
                              ),
                            );
                          }
                        ...
                        child: TextButton(
                          onPressed: () {
                            UploadHtmlHelper().startUpload(
                              productDetailController.uploadImageProduct,
                            );
                          },
                          ...
                          child: Observer(
                            builder: (_) {
                              return Text(
                                '${productDetailController.imagePath == null ? 'Adicionar' : 'Alterar'} Foto',
                              );
                            },
                          ),
                        ),
                              ...
                              showDialog(
                                ...
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    content: Text(
                                      'Confirma a exclusão do produto ${productDetailController.productModel!.name}',
                                    ),
                                    actions: [
                                      TextButton(
                                        onPressed: () {
